const getClass=async manifest=>{return class{#prop;#api;constructor(api,options){this.#prop={options:options||{},manifest:manifest},this.#api=api}init(){this.#api.menubar([{label:"File",items:[{label:"New",action:()=>{this.newCalc()}}]}]),this.newCalc()}event(eventName,eventData){"changedWindow"===eventName&&eventData&&$("input.calc-input",eventData.window()).focus()}newCalc(){const w=this.#api.newWindow(newCalcOptions);makeCalc(this,w.window()),w.show()}}},newCalcOptions={minWidth:"260px",minHeight:"180px",width:"320px",height:"200px"},makeCalc=(co,$c)=>{const $out=$(`<div class="p-2">
