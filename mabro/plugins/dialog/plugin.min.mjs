const getClass=async mb=>{const DLOG=class{#prop;#dlog;constructor(manifest,options){const dlogDefaults={uid:glob.uid("dlog"),canclose:!0,size:"lg",tooltip:"",title:"",body:"",footer:""};if("object"!=typeof options)options=dlogDefaults;else{const okeys=Object.keys(options),dkeys=Object.keys(dlogDefaults);dkeys.filter(k=>!okeys.includes(k)).forEach(k=>{options[k]=dlogDefaults[k]})}this.#prop=options}static makeDlog(p){return $(`<div class="modal fade mabro-global-system-modal" id="${p.uid}" tabindex="-1" role="dialog" aria-labelledby="${p.tooltip}" data-backdrop="${!!p.canclose||"static"}" data-keyboard="${!!p.canclose}" aria-hidden="true">
